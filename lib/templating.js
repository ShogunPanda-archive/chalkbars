...
// --- Handlebars helpers ---
handlebars.registerHelper('cb', function(){
...
handlebars.registerHelper('cbh', function(){
  if(!arguments.length){
    return "";
  }
  ...
  // If there was only an argument, it's considered to be the content and the styles are empty
  if(!embedded){
    embedded = styles;
    styles = "";
  }
  ...
  var embedded = handlebars.helpers.cb(styles, {
    fn: function(){
...
handlebars.registerHelper('cbf', function(){
  if(!arguments.length){
    return "";
  }
...
var renderTemplate = module.exports = function(args, context){
  // Join arguments. If all the handlebars arguments for cb* are unquoted, transform it as a single quoted argument.
  var sources = args.map(function(arg){
    return typeof arg == 'object' ? JSON.stringify(arg) : arg.toString();
  }).join('').replace(/\{\{(#cb|cbh|cbf) ([^"].*?)\}\}/g, '{{$1 "$2"}}');
